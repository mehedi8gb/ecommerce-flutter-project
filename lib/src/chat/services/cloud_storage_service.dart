    ...
    try {
      return _baseRef
          ...
          .putFile(_file).then((value) async {
            ...
            if(toId != null) {
              if(notify) {
                ApiProvider().post('/wp-admin/admin-ajax.php?action=build-app-online-new_chat_message', {'message_url': _imageURL, 'vendor_id': toId});
              }
            } else if(notify) {
              ApiProvider().post('/wp-admin/admin-ajax.php?action=build-app-online-new_chat_message', {'message_url': _imageURL});
              notify = false;
            }
          });
    } catch (e) {
