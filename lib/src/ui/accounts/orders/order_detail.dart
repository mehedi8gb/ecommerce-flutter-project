...
class _OrderDetailState extends State<OrderDetail> {
  ...
  _buildList(BuildContext context) {
      ...
      item.metaData.forEach((element) {
        if(element.value is String && !element.key.contains('_'))
          list.add(Container(
              ...
              child: Text(element.key + ': ' + element.value, style: _subtitleTextStyle(theme, tileTheme)))
          );
      });
    }
    ...
    if(widget.appStateModel.blocks.vendorType == 'wcfm')
    list.add(
      ButtonBar(
        alignment: MainAxisAlignment.center,
        children: [
          if(widget.order.status == 'completed' || widget.order.status == 'processing' || widget.order.status == 'ready-delivery' || widget.order.status == 'on-hold')
          ...
          if(widget.order.status == 'on-hold')
          ...
          if(widget.order.deliveryBoy.phone.isNotEmpty)
            OutlinedButton(onPressed: () {
              launch('tel:'+widget.order.deliveryBoy.phone);
            }, child: Text('Call Delivery Boy'))
        ],
      ),
    );
    else if(widget.order.status == 'on-hold' || widget.order.status == 'processing')
      list.add(
        ButtonBar(
          alignment: MainAxisAlignment.center,
          children: [
