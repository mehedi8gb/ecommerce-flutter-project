...
import '../../../../assets/fonts/presentation/m_store_icons_icons.dart';
...
class _ProductDetail2State extends State<ProductDetail2> {
        ...
        builder: (context, snapshot) {
          if (snapshot.hasData) {
            return Scaffold(
              ...
              body: Builder(
                  builder: (context) {
                    if (widget.appStateModel.blocks.settings.productFooterAddToCart) {
                      return Stack(
                        children: [
                          CustomScrollView(
  ...
  _buildSlivers(BuildContext context, Product product) {
    ...
    if (product.availableVariations.isNotEmpty &&
        product.availableVariations.length != 0 && !widget.appStateModel.blocks.settings.productFooterAddToCart) {
    ...
    } else if(widget.appStateModel.blocks.settings.catalogueMode != true) {
            ...
            return GestureDetector(
              ...
              child: Chip(
                ...
                backgroundColor: variationOption.selected ==
                    variationOption.options[index] ? Theme.of(context).colorScheme.secondary : Colors.black12,
                label: Text(
      ...
      if (product.type == 'variable' &&
        ...
        for (var i = 0; i < product.variationOptions.length; i++) {
          if (product.variationOptions[i].selected != null) {
            data['variation[attribute_' + product.variationOptions[i].attribute.toLowerCase().replaceAll(' ', '-').replaceAll("'", "") + ']'] = product.variationOptions[i].selected;
            data['attribute_pa_' + product.variationOptions[i].attribute.toLowerCase().replaceAll(' ', '-').replaceAll("'", "")] = product.variationOptions[i].selected;
        ...
        children: <Widget>[
          ...
          Text(
              (product.formattedPrice != null)
                  ...
                  : '',
              style: onSale && (product.formattedSalesPrice != null) ? Theme.of(context).textTheme.caption!.copyWith(
